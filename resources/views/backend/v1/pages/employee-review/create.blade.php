...
@section('title')
<h2 class="text-white pb-2 fw-bold">Tambah Telaah Staff</h2>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('employee-review.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="number">No Surat</label>
                <input type="text" class="form-control" readonly name="number" id="number"
                    placeholder="Masukkan No Surat" value="{{ $employee_review ? $employee_review->number+1 : '1' }}" required>
            </div>
            <div class="form-group">
