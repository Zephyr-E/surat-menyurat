...
@section('title')
<h2 class="text-white pb-2 fw-bold">Ubah Surat Keputusan</h2>
@endsection

@section('content')
<div class="card">
    <div class="card-body">
        <form action="{{ route('decree.update', $decree) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('patch')
            <div class="form-group">
                <label for="number">No Surat</label>
                <input type="text" class="form-control" readonly name="number" id="number"
                    placeholder="Masukkan No Surat" value="{{ $decree->number }}" required>
            </div>
            <div class="form-group">
                <label for="regarding">Perihal</label>
                <textarea class="form-control" name="regarding" id="regarding" cols="30" rows="2"
                    placeholder="Isi Perihal" required>{{ $decree->regarding }}</textarea>
            </div>
            <div class="form-group">
                <label for="date">Tanggal</label>
                <input type="date" class="form-control" name="date" id="date" value="{{ $decree->date }}" required>
            </div>
            <div class="form-group">
                ...
                <select class="form-control selectpicker" id="employee_id" name="employee_id" data-live-search="true"
                    ...
                    <option value="{{ $employee->id }}" {{ ($employee->id == $decree->employee->id) ? 'selected'
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="disposition" id="disposition"
                    placeholder="Masukkan Disposisi Surat" value="{{ $decree->disposition }}" required>
            </div>
            <div class="form-group">
                <label for="file">Upload File <small class="text-danger">(format .pdf maks. 10MB)</small></label>
                <input type="hidden" name="oldFile" value="{{ $decree->file }}">
